...
%PATCH_CCS_FILES Utility to patch files in a CCS 10 installation
%   Provides a mechanism to patch files in a CCS 10 installation in a way
%   which:
...
%     patch was created.
    ...
    if nargin == 0
        ccs_install_root = uigetdir ('' ,'Select CCS 10 installation root');
        if isequal (ccs_install_root, 0)
            return
        end
    end
    
    if ~exist(fullfile (ccs_install_root, 'ccs/ccs_base'), 'dir')
        fprintf ('%s is not a the root directory of a CCS 10 installation\n', ccs_install_root);
        return
    end

    % Set the context containing paths common to all file patched
    context.ccs_install_root = fullfile (ccs_install_root, 'ccs');
    context.ccs_backup_root = fullfile (ccs_install_root, 'ccs_original_files');
